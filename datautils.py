...
class SASDataloader():
    # modified by ssgds (add infer, D225374)
    ...
    def get_pytorch_dataloaders(self):
        ...
        infer_loader = self._get_infer_loader()
    ...
    # added by ssgds (D225374)
    def _get_infer_loader(self):
        dataset = self._get_infer_dataset()
        dataloader = data_utils.DataLoader(dataset, batch_size=self.args.infer_batch_size,
                        shuffle=True, pin_memory=True, num_workers=self.args.num_workers,
                        worker_init_fn=worker_init_fn)
        return dataloader
    
    def _get_infer_dataset(self):
        dataset = SASInferDataset(
            self.args, self.infer, self.max_len, self.rng)
        return dataset
...
# added by ssgds (D225374)
class SASInferDataset(data_utils.Dataset):
    def __init__(self, args, u2seq, max_len, rng):
        self.args = args
        self.u2seq = u2seq
